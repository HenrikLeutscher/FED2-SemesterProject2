    ...
    .map((listing) => {
      ...
      return `
        <div class="border border-blue-400 rounded mb-5">
            <img loading="lazy" src="${imageSrc}" class="w-full h-50 object-cover mb-4" alt="${
        listing.title
            ...
            <div class="px-4 py-2">
                ...
                <div class="flex justify-between">
                    <p>${bidCount} bids</p>
                    <p class="countdown" data-ends="${
                      listing.endsAt
                    }">Loading...</p>
                </div>

                <a href="/pages/individualAuctionListing/individualAuctionListing.html?id=${
                  listing.id
                }" class="w-full bg-blue-600 text-white px-4 py-2 rounded-2xl hover:bg-blue-900 transition border-white border block mx-auto text-center">
                  View Details
                </a>
            </div>
        </div>`;
